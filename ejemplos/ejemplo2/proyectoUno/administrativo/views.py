...
def editar_numero_telefonico(request, id):
    ...
    return render(request, 'crearNumeroTelefonico.html', diccionario)
def ver_numero_telefonico(request, id):
    """
    Ver los detalles de un número de teléfono.
    """
    telefono = NumeroTelefonico.objects.get(pk=id)
    informacion_template = {'telefono': telefono}
    return render(request, 'verNumeroTelefonico.html', informacion_template)
...
def eliminar_numero_telefonico(request, id):
    ...
    telefono = NumeroTelefonico.objects.get(pk=id)
    telefono.delete()
    messages.success(request, "Número de teléfono eliminado exitosamente.")
    return redirect(index)
